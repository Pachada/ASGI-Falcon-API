...
def qualify_picks(pool_user: Pool_User, matches_count: int):
...
def qualify_tiebreakers(pool_user: Pool_User, matches_count: int):
...
def foo(pool_user: Pool_User, matches_count: int):
    score = 0
    # Revizamos los picks
    score +=  qualify_picks(pool_user, matches_count)

    # Revizamos los tiebreakers
    score +=  qualify_tiebreakers(pool_user, matches_count)
...
def qualify_pool(pool):
    ...
    if isinstance(pool, Pool):
        ...
        result = [foo(pool_user, matches_count) for pool_user in pool_users]
